...
contract PriceOracle is IPriceOracle, Affinity {
    ...
    /// #if_succeeds {:msg "Price precision is more than 18"} getTokenPrice(tokenAddr) > 0;
    ...
    /// #if_succeeds {:msg "Price precision is more than 18"} getTokenPrice(baseToken) > 0 && getTokenPrice(quoteToken) > 0;
    ...
    /// #if_succeeds {:msg "tokenAddr is not empty"} tokenAddrs.length > 0;
    /// #if_succeeds {:msg "Price oracle mode updated for all tokens"} forall(uint i in 0...tokenAddrs.length-1) priceOracleModeMap[tokenAddrs[i]] == mode;
    ...
    /// #if_succeeds {:msg "Caller is a savior"} msg.sender == SAVIOR;
    /// #if_succeeds {:msg "tokenAddr is not empty"} tokenAddrs.length > 0;
    /// #if_succeeds {:msg "Feed contracts updated for all tokens"} forall(uint i in 0...tokenAddrs.length-1) spareFeedContracts[tokenAddrs[i]] == feedContracts[i];
    ...
    /// #if_succeeds {:msg "Caller is a savior"} msg.sender == SAVIOR;
    /// #if_succeeds {:msg "tokenAddr is not empty"} tokenAddrs.length > 0;
    /// #if_succeeds {:msg "Router info map updated for all tokens"} forall(uint i in 0...tokenAddrs.length-1) tokenUniv3RouterInfoMap[tokenAddrs[i]].swapRouter == uniV3RouterInfos[i].swapRouter;
    ...
    /// #if_succeeds {:msg "Caller is a savior"} msg.sender == SAVIOR;
    /// #if_succeeds {:msg "Twap interval updated"} twapInterval == _twapInterval;
    function setTwapInterval(uint256 _twapInterval) external isSavior {
        twapInterval = _twapInterval;
    }

    /// #if_succeeds {:msg "Caller is a savior"} msg.sender == SAVIOR;
    /// #if_succeeds {:msg "Price updated"} prices[tokenAddr] == price;
    ...
    /// #if_succeeds {:msg "swapRouter is not zero Address"} tokenUniv3RouterInfoMap[token].swapRouter != address(0);
