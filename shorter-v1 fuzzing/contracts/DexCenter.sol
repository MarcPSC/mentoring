...
contract DexCenter is Affinity, IDexCenter {
    ...
    /// #if_succeeds {:msg "Ammount in allowance limit"} ISRC20(params.path.getRouter()[0]).allowance(address(this), params.swapRouter) < params.amountIn;
    /// #if_succeeds {:msg "Tokens sold"} ISRC20(params.path.getRouter()[0]).balanceOf(address(this)) + params.amountIn == old(ISRC20(params.path.getRouter()[0]).balanceOf(address(this))) && ISRC20(params.path.getRouter()[params.path.getRouter().length - 1]).balanceOf(params.to) - usdAmount == old(ISRC20(params.path.getRouter()[params.path.getRouter().length - 1]).balanceOf(params.to));
    ...
    /// #if_succeeds {:msg "Placeholder"} true;
    function buyCover(BuyCoverParams memory params) external returns (uint256 amountIn) {
        ...
        /// #assert {:msg "Ammount in allowance limit"} ISRC20(address(tokenIn)).allowance(address(this), params.swapRouter) < params.amountInMax;
        /// #assert {:msg "Tokens sold"} tokenInAft + amountIn == tokenInBal && tokenOutBal + params.amountOut == tokenOutAft;
    ...
    /// #if_succeeds {:msg "Valid swapRouter"} isSwapRouterV3[swapRouter];
    ...
    /// #if_succeeds {:msg "Valid path length"} path.getRouter().length >= 2 && path.getRouter().length < 5;
    /// #if_succeeds {:msg "Valid first and last token in path"} isSell || isSwapRouterV3[swapRouter] ==> path.getRouter()[0] == token0 && path.getRouter()[path.getRouter().length.sub(1)] == token1;
    /// #if_succeeds {:msg "Valid first and last token in path"} !(isSell || isSwapRouterV3[swapRouter]) ==> path.getRouter()[0] == token1 && path.getRouter()[path.getRouter().length.sub(1)] == token0;
    /// #if_succeeds {:msg "All middle tokens are middle tokens"} path.getRouter().length > 2 ==> forall(uint i in 1...path.getRouter().length-1) isMiddleToken[path.getRouter()[i]];
    function checkPath(
        address token0,
        address token1,
        address swapRouter,
        ...
        /// #assert isSell || isSwapRouterV3[swapRouter] ==> _path[0] == token0 && _path[pathSize.sub(1)] == token1;
    }
    ...
    /// #if_succeeds {:msg "Entitled swap router updated"} forall(uint i in 0..._swapRouters.length-1) isSwapRouterV3[_swapRouters[i]] == _isSwapRouterV3;
    ...
    /// #if_succeeds {:msg "Updated Twap interval"} twapInterval == newTwapInterval;
