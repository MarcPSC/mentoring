...
contract UniClassDexCenter is BaseDexCenter {
    ...
    /// #if_succeeds {:msg "swapRouter is in entitledSwapRouters"} entitledSwapRouters[swapRouter];
    /// #if_succeeds {:msg "Uniswap v3 path valid"} path.length > 2 ==> forall(uint i in 1...path.length-2) isMiddleToken[path[i]];
    ...
    /// #if_succeeds {:msg "swapRouter is in entitledSwapRouters"} entitledSwapRouters[swapRouter];
    /// #if_succeeds {:msg "Uniswap v3 path valid"} Path.getRouter(exactInputParams.path).length > 2 ==> forall(uint i in 1...Path.getRouter(exactInputParams.path).length-2) isMiddleToken[Path.getRouter(exactInputParams.path)[i]];
    ...
    /// #if_succeeds {:msg "swapRouter is in entitledSwapRouters"} entitledSwapRouters[swapRouter];
    /// #if_succeeds {:msg "Uniswap v3 path valid"} Path.getRouter(exactOutputParams.path).length > 2 ==> forall(uint i in 1...Path.getRouter(exactOutputParams.path).length-2) isMiddleToken[Path.getRouter(exactOutputParams.path)[i]];
