...
contract TreasuryImpl is ChainSchema, TreasuryStorage {
    ...
    /// #if_succeeds !old(_initialized);
    /// #if_succeeds _initialized;
    /// #if_succeeds _threshold > 0;
    /// #if_succeeds threshold == _threshold;
    ...
    /// #if_succeeds {:msg "Caller is a savior"} msg.sender == SAVIOR;
    /// #if_succeeds {:msg "Owners list added"} forall(uint i in 0..._owners.length-1) owners.contains(_owners[i]);
    ...
    /// #if_succeeds {:msg "Caller is a savior"} msg.sender == SAVIOR;
    /// #if_succeeds {:msg "Owner removed"} !owners.contains(_owner);
    ...
    /// #if_succeeds {:msg "Threshold updated"} threshold == newThreshold;
    ...
    /// #if_succeeds {:msg "Separator returned"} $result == keccak256(abi.encode(DOMAIN_SEPARATOR_TYPEHASH, getChainId(), this));
    function _domainSeparator() internal view returns (bytes32) {
        return keccak256(abi.encode(DOMAIN_SEPARATOR_TYPEHASH, getChainId(), this));
    }

    /// #if_succeeds {:msg "Owner setted"} owners.contains(_owner);
     ...
     */
    /// #if_succeeds {:msg "Signature's length correct"} signatures.length >= threshold.mul(65);
    /// #if_succeeds {:msg "Signature correct"} forall(uint i in 1...threshold-1) PermitLibrary.getSigner(signatures, i, dataHash) > PermitLibrary.getSigner(signatures, i-1, dataHash) && owners.contains(PermitLibrary.getSigner(signatures, i, dataHash)) && PermitLibrary.getSigner(signatures, i, dataHash) != address(0);
    /// #if_succeeds {:msg "Signature correct"} PermitLibrary.getSigner(signatures, 0, dataHash) > address(0) && owners.contains(PermitLibrary.getSigner(signatures, 0, dataHash)) && PermitLibrary.getSigner(signatures, 0, dataHash) != address(0);
