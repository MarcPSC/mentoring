...
async function main() {
  ...
  const usdt = await Usdt.connect(savior).deploy();
  await usdt.deployed();

  const latestBlock = await hre.ethers.provider.getBlock("latest");

  wrappedtokenimpl.connect(savior).initialize("Wrapped Ether", "WETH", 18);

  Factory = new ContractFactory(artifacts.UniswapV3Factory.abi, artifacts.UniswapV3Factory.bytecode, savior);
  factory = await Factory.deploy();
  NFTDescriptor = new ContractFactory(artifacts.NFTDescriptor.abi, artifacts.NFTDescriptor.bytecode, savior);
  nftDescriptor = await NFTDescriptor.deploy();
  const nonfungiblePositionManager = noFungibleUniswap(savior, nftDescriptor, wrappedtokenimpl);
  (await nonfungiblePositionManager).deployed()
  
  committeeimpl.connect(savior).initialize(shorterbone.address, ipistrtokenimpl.address, usdt.address);
  auctionhallimpl.connect(savior).initialize(shorterbone.address, ipistrtokenimpl.address, poolguardianimpl.address,tradinghubimpl.address, priceoracle.address, committeeimpl.address, wrappedtokenimpl.address, latestBlock+1000, latestBlock+10000000);
  farmingimpl.connect(savior).initialize(shorterbone.address, nonfungiblePositionManager.address);
  shorterfactory.connect(savior).setShorterBone(shorterbone);
  wrappedtokenimpl.connect(savior).setMinter(wethWallet, true);

  usdt.mint(usdtWallet, 1000);
  wrappedtokenimpl.connect(savior).mint(wethWallet, 1000);

  shorterbone.connect(savior).setAlly(utils.keccak256(utils.toUtf8Bytes("POOL_GUARDIAN")), poolguardianimpl.address);
  await shorterfactory.createStrPool(1);
